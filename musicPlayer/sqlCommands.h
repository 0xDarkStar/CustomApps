...
int add_song_to_table(int id, std::string title, std::string artist, bool has_subs, int length) { // length is in seconds
    sqlite3* DB;
    char* messaggeError;
    int exit = 0;
...
int get_table_length(std::string table) {
    ...
    int exit;
    sqlite3_open("main.db", &DB);
    std::string sql = "SELECT COUNT(ID) FROM " + table + ";";
